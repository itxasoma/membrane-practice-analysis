...
if __name__ == '__main__':
    ...
    raw_plot(t, data['TEMP'], C_T, T_TARGET,
             r'Temperature (K)',
             os.path.join(FIG_DIR, '1.temperature_304.pdf'),
             rf'$T_{{\rm target}} = {T_TARGET}$ K')

    zscore_plot(t, data['TEMP'], C_T, T_TARGET,
                r'Temperature (K)',
                os.path.join(FIG_DIR, '1.temperature-zscore_304.pdf'),
                rf'$T_{{\rm target}} = {T_TARGET}$ K')

    raw_plot(t, data['VOLUME'], C_V, v0,
             r'Volume ($\mathrm{\AA}^3$)',
             os.path.join(FIG_DIR, '1.volume_304.pdf'),
             rf'$V_0 = {v0:.1f}\ \mathrm{{\AA}}^3$')

    zscore_plot(t, data['PRESSURE'], C_P, P_TARGET,
                r'Pressure (bar)',
                os.path.join(FIG_DIR, '1.pressure-zscore_304.pdf'),
                rf'$P_{{\rm target}} = {P_TARGET}$ bar')

    raw_plot(t, data['PRESSURE'], C_P, P_TARGET,
             r'Pressure (bar)',
             os.path.join(FIG_DIR, '1.pressure_304.pdf'),
             rf'$P_{{\rm target}} = {P_TARGET}$ bar')

    # ── Block-averaged uncertainties in txt ───────────────────────────────────

    half = len(t) // 2
    out_txt = os.path.join(FIG_DIR, '1.block_stats_304.txt')
